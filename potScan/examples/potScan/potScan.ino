...
#include "potScan.h";                           // include the library
potScan scanPot = potScan();                    // define scanPot

void setup() {
  pinMode(9, INPUT_PULLUP);
  Serial.begin(115200);
  scanPot.begin(A0, 127);
  // scanPot.begin(int potPin, int maxVal, bool forceOutput) the forceOutput argument is optional! (false by default)
}

void loop() {
  int potVal = scanPot.getVal();
  // get the current potentiometer value within the defined range, in hysteresis protected steps, without any jitter
  if (potVal >= 0)  Serial.println(potVal, DEC);
  if (digitalRead(9) == LOW) {
    parameterFeedback();
    while (!digitalRead(9)) delay(100); // debounce
  }
}

void parameterFeedback() {
  Serial.println(scanPot.getVal(true), DEC);
  // scanPot.getVal(bool recall) the recall argument is optional! (false by default)
}
